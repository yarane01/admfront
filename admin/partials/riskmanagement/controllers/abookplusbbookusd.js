angular
    .module('portal')
    .controller('RiskManagementAbookPlusBbookUSDCtrl', RiskManagementAbookPlusBbookUSDCtrl);

function RiskManagementAbookPlusBbookUSDCtrl($scope, $rootScope, RiskManagementService,$timeout) {
    ...
    $scope.tableSize = '25vh';
    $scope.tableProperties = tableProperties.riskmanagement_abookplusbbookusd;

    $scope.clearFilter = function() {
        $('#abookplusbbookusdfilter').val('');
    ...
    $scope.afterTableCreated = function () {
        $timeout(function () {
            ...
            var searchfield =  $('#abookplusbbookusdfilter');
            ...
            var buttons = $scope.table.buttons().container().find('.btn');
             ...
             $('#abookplusbbookusd-panel-title').find('.export-placeholder').after(buttons);//replaceWith(buttons);
        }, 0);
    };
    ...
    function createData() {
        ...
        RiskManagementService.getTotalItems().forEach(function (item) {
            if (item.source == 'Sum') {
                $scope.items.push({
